...
import kr.co.gymleco.domain.banner.SectionKey;
...
public class AdminBannerController {
    ...
    @GetMapping("/section-keys")
    public List<Map<String, String>> sectionKeys(){
        return Arrays.stream(SectionKey.values()).map(k -> Map.of("value", k.key(), "label", k.label())).toList();
    }
    @PutMapping("/section-media/{key}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void putSection(@PathVariable String key, @Valid @RequestBody SectionMediaRequest body, @AuthenticationPrincipal Jwt jwt, HttpServletRequest http) {
        SectionKey.of(key).orElseThrow(() -> new IllegalArgumentException("알 수 없는 구역입니다: " + key));
