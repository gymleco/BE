...
public record FaqAdminResponse(
    ...
    public static FaqAdminResponse from(Faq f) {
        return new FaqAdminResponse(f.getId(), f.getCategory(), f.getQuestion(),
            f.getAnswer(), f.getSortOrder(), f.isVisible(), f.getUpdatedAt());
    }
}
