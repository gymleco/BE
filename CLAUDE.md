...
| 대상 | 방식 |
|---|---|
| **`.java` — 새 로직·설계** | **디스크에 쓰지 않는다.** 채팅으로 출력하고 개발자가 직접 검수·타이핑한다. |
| **`.java` — 기계적 수정** | Claude 가 직접 수정한다. 무엇을 왜 고쳤는지 함께 설명한다. |
| **그 외 전부** | Claude 가 직접 작성한다 — `build.gradle`, `application.yml`, `Dockerfile`, `compose.yaml`, **마이그레이션 SQL**, nginx·k8s 설정, 디렉터리 구조 |

**새 로직은 채팅으로 출력한다.** 백엔드 로직은 개발자가 직접 타이핑하며
익히기 위한 것이므로, 파일을 만들어 버리면 목적이 사라진다.

**기계적 수정은 Claude 가 직접 고친다.** 애너테이션 누락, import 추가,
한 줄 버그 수정처럼 배울 것이 없는 변경까지 타이핑하는 것은 시간만 쓴다.
경계가 애매하면 "이걸 타이핑하면서 새로 배울 것이 있는가" 로 판단한다.
